// Widget embebible minimalista para insertar en otras páginas
(function(){
    ...
    function renderWidget(target, options = {}) {
        ...
        const agentId = options.agentId || 'YOUR_AGENT_ID'; // Permite pasar agentId dinámicamente
        ...
        widget.innerHTML = `
            ...
            <button id="widget-bot-toggle" style="background:#2e7dff;color:#fff;border:none;border-radius:8px;padding:10px 18px;font-size:1em;cursor:pointer;transition:background 0.2s;">🎙️ Iniciar conversación</button>
        ...
        if (elevenlabsContainer) {
            ...
            el.setAttribute('dynamic-variables', JSON.stringify({ pagina, nombre, modalidad, capital, valor_cuota, moneda, Numero_Grupo, Numero_Orden, Mes_Grupo }));
            elevenlabsContainer.appendChild(el);
            // Asegura que el script de ElevenLabs esté cargado
        ...
        toggleBtn.onclick = async function() {
            if (isActive) {
                ...
                conversation = null;
                isActive = false;
                toggleBtn.innerHTML = '🎙️ Iniciar conversación';
                updateStatus(false);
            } else {
                const hasMic = await requestMic();
                if (!hasMic) return alert('Necesito acceso al micrófono.');
                const signedUrl = await getSignedUrl();
                if (!signedUrl) return alert('No se pudo iniciar la conversación.');
                if (!window.Conversation) return alert('No se encontró Conversation.');
                conversation = await window.Conversation.startSession({
                    signedUrl,
                    dynamicVariables: { pagina, nombre, modalidad, capital, valor_cuota, moneda, Numero_Grupo, Numero_Orden, Mes_Grupo },
                    onConnect: () => {
                        updateStatus(true);
                        toggleBtn.innerHTML = '⏹️ Finalizar conversación';
                        isActive = true;
                    },
                    onDisconnect: () => {
                        updateStatus(false);
                    ...
                    onError: () => {
                        updateStatus(false);
                        alert('Error en la conversación.');
                    }
                });
            }
        };
